...
source ${qtoolScriptDir_Absolute}/src/framework_category_util.sh
source ${qtoolScriptDir_Absolute}/src/branch_function.sh
project_dir=$(get_sysenv_project_dir)

# 获取参数文件路径（优先命令行参数，其次环境变量）
...
branch_type_menu() {
    ...
    if [ -z "${branchBelongMaps1}" ] || [ "${branchBelongMaps1}" == "null" ]; then
        rebaseErrorMessage="您正在为 ${project_dir} 项目操作分支，但未找到可选的操作类型，请先在 ${target_branch_type_file_abspath} 文件中设置 .${branchBelongKey1} "
        printf "${RED}%s${NC}\n" "${rebaseErrorMessage}"
        exit 1
    fi
...
# 1.2、分支改动范围参考及输入
# 初始化文件路径
target_category_file_abspath=$(getCategoryFile "${target_branch_type_file_abspath}")
if [ $? != 0 ]; then
    echo "${target_category_file_abspath}"
    exit 1
fi

target_person_file_abspath=$(getPersonFile "${target_branch_type_file_abspath}")
if [ $? != 0 ]; then
    echo "${target_person_file_abspath}"
    exit 1
fi

show_and_get_framework_category "${target_category_file_abspath}" "${target_person_file_abspath}"

# TODO: 用户交互部分待需改为使用 chooseAndCompleteBranchName
# 无限循环，监听用户输入
while true; do
