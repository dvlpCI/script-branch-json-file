...
2. **Info.plist**：`QBMenuSources` 数组（`file` + `type` + `name`），`file` 为绝对路径，app 运行时直接从该路径读取 JSON
...
```swift
// JSON 路径解析（QBMenuSources 存绝对路径，直接读取）：
// - file 以 / 开头 → 直接使用（Info.plist 中的绝对路径）
// - 否则 → 搜索 resourcesDir、binDir、brew lib 路径
```
...
└── Info.plist                  # 含 QBMenuSources 数组声明菜单源（含 JSON 绝对路径）
```

Info.plist 中的 `QBMenuSources` 数组声明每个菜单源的路径、类型、显示名（`file` 为绝对路径，app 运行时直接读取源文件，修改 JSON 后退出重开即生效）：
```xml
<key>QBMenuSources</key>
<array>
    <dict>
        <key>file</key><string>/Users/qian/Project/Book/qbase_custom_menu.json</string>
        <key>type</key><string>custom</string>
        <key>name</key><string>qbase</string>
    </dict>
    <dict>
        <key>file</key><string>/Users/qian/Project/Github/script-branch-json-file/qtool_menu_public.json</string>
...
注意：**JSON 文件不嵌入 .app**，运行时从 Info.plist 记录的绝对路径直接读取，修改 JSON 后退出重开 Qtool 即可生效。业务脚本仍在 Homebrew Cellar 中。
...
3. 拷贝 wrapper 脚本到 `Resources/`
4. 写入 `Info.plist`（含 `QBMenuSources`，`file` 为 JSON 的绝对路径）
5. 输出到目标路径（默认桌面）
6. 清理临时文件
...
| 0.2.1 | 2026-06-25 | JSON 不嵌入 .app，改存绝对路径，修改 JSON 后退出重开即生效 |
