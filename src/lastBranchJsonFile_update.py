...
LastEditTime: 2023-04-19 01:21:51
...
def choose_last_branchs_info_file_path():
    ...
    # print("文件路径2：", last_branchs_info_dir_path)
    ...
    lastBranchsInfo_files = [os.path.join(last_branchs_info_dir_path, f) for f in os.listdir(last_branchs_info_dir_path) if f.endswith('.json')]
    ...
    while True:
        ...
        if user_input.lower() == 'q':
            ...
            print(f"{RED}输入的不是JSON文件名，请重新输入{NC}")
            continue
        elif user_input not in [os.path.basename(file) for file in lastBranchsInfo_files]:
            print(f"{RED}文件不存在，请重新输入{NC}")
            continue
        else:
            last_branchs_info_file_path = os.path.join(last_branchs_info_dir_path, user_input)
