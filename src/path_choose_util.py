...
LastEditTime: 2023-06-13 15:31:43
...
def show_and_choose_folder_in_dir(searchInDir):
    # 获取第一层文件夹名，如果是文件则不需要
    folder_names = [dir for dir in os.listdir(searchInDir) if os.path.isdir(os.path.join(searchInDir, dir))]
    ...
    while True:
        user_input = input("请输入想要操作的文件夹名（输入Q或q退出）：")
        if user_input.lower() == 'q':
            ...
            git_project_folder_path = joinFullPath_checkExsit(searchInDir, user_input)
...
def show_and_choose_file_in_dir(searchInDir, fileExtension, GiveupChoose=False):
    ...
    lastBranchsInfo_files = [joinFullPath_checkExsit(searchInDir, f) for f in os.listdir(searchInDir) if f.endswith(fileExtension)]
    ...
    while True:
        ...
        if user_input.lower() == 'q':
            ...
            last_branchs_info_file_path = joinFullPath_checkExsit(searchInDir, user_input)
...
def copy_special_file_inDir_toDir(source_dir_path, copy_file_extension, backup_to_dir_path):
    # 获取符合条件的文件列表
    try:
        apk_files = [filename for filename in os.listdir(source_dir_path) if filename.endswith(copy_file_extension)]
