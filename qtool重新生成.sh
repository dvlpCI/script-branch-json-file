...
# 定义颜色常量(qtool 不是所要执行的直接脚本，所以不要使用颜色)
NC="\033[0m" # No Color
RED="\033[31m"
GREEN="\033[32m"
YELLOW="\033[33m"
...
# 删除 cript-name.x.c
rm -rf ${qtool_HomeDir_Absolute}/qtool.sh.x.c
# 重命名
mv ${qtool_HomeDir_Absolute}/qtool.sh.x ${qtool_HomeDir_Absolute}/qtool

# 检查生成的二进制文件是否可执行
if [ ! -x "${qtool_HomeDir_Absolute}/qtool" ]; then
    echo "${RED}Error：生成的 qtool 文件不可执行${NC}"
    exit 1
fi
