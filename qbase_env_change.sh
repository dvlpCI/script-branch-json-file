...
log_color_info "${YELLOW}正在执行命令《${BLUE} sh ${qbase_env_file_check_and_set_scriptPath} --env-name \"${CHOICES_ENV_VAR}\" --env-descript qtool可操作的项目列表 --env-var-placeholder \"${PLACEHOLDER}\" --env-reference-json-file-example ${example_json_file_choices} --output-filename-if-copy tool_choice.json ${YELLOW}》。${NC} "
checkResult=$(sh ${qbase_env_file_check_and_set_scriptPath} \
    --env-name "${CHOICES_ENV_VAR}" \
    --env-descript qtool可操作的项目列表 \
    --env-var-placeholder "${PLACEHOLDER}" \
...
sh "${qtoolScriptDir_Absolute}/qbase_env_check.sh" \
    "${qtoolScriptDir_Absolute}" \
    --choices-env "${CHOICES_ENV_VAR}" \
    --target-env "${TARGET_ENV_VAR}" \
    --placeholder "${PLACEHOLDER}" \
    --action "${ACTION}" \
    --desc-not-set "${DESC_NOT_SET}" \
    --desc-file-not-found "${DESC_FILE_NOT_FOUND}"
